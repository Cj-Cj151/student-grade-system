...
$users = $pdo->query("
    ...
    FROM users u
    LEFT JOIN students s ON s.user_id = u.user_id AND u.role = 'student'
    LEFT JOIN teachers t ON t.user_id = u.user_id AND u.role = 'teacher'
...
<div class="glass-card panel">
    ...
    <?php if (empty($users)): ?>
        <div class="empty-state"><p>No user accounts found.</p></div>
    <?php else: ?>
        <div class="table-wrap">
            <table class="data-table" id="usersTable">
                <thead><tr><th>Login ID</th><th>Name</th><th>Role</th><th>Status</th><th>Created</th><th>Actions</th></tr></thead>
                <tbody>
                    <?php foreach ($users as $u): ?>
                        <tr data-search="<?= clean(strtolower($u['login_id'] . ' ' . $u['first_name'] . ' ' . $u['last_name'])) ?>"
                            ...
                            <td class="row-actions">
                                <button type="button" class="btn btn-secondary btn-sm" onclick="openResetModal(<?= (int) $u['user_id'] ?>, '<?= clean($u['login_id']) ?>')">Reset Password</button>
                                <?php if ($u['user_id'] != $_SESSION['user_id']): ?>
                                    <button type="button" class="btn btn-sm <?= $u['is_active'] ? 'btn-danger' : 'btn-primary' ?>"
                                        onclick="toggleStatus(<?= (int) $u['user_id'] ?>, <?= $u['is_active'] ? 'false' : 'true' ?>)">
        ...
        <p class="empty-state" id="noResultsState" style="display:none;">No users match your search or filter.</p>
    <?php endif; ?>
</div>

<div class="modal-overlay" id="resetModal">
    <div class="glass-card modal-box" style="max-width:400px;">
        <div class="modal-header">
            <h3 class="modal-title">Reset Password</h3>
            <button type="button" class="modal-close" onclick="closeModal('resetModal')">&times;</button>
        </div>
        <div id="resetModalAlert" class="alert alert-error"></div>
        <form id="resetForm">
            <input type="hidden" id="reset_user_id" value="">
            <div class="form-group">
                <label>Login ID</label>
                <input type="text" id="reset_login_id" class="form-control" disabled>
            </div>
            <div class="form-group">
                <label for="new_password">New Password</label>
                <input type="text" id="new_password" class="form-control" placeholder="At least 6 characters" required>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" onclick="closeModal('resetModal')">Cancel</button>
                <button type="submit" class="btn btn-primary" id="resetSaveBtn">Reset Password</button>
            </div>
        </form>
    </div>
</div>
...
async function toggleStatus(userId, makeActive) {
    const ok = await confirmAction(
        makeActive ? 'Activate this account?' : 'Deactivate this account?',
...
function openResetModal(userId, loginId) {
    ...
    document.getElementById('resetModalAlert').classList.remove('show');
    openModal('resetModal');
}

document.getElementById('resetForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    const alertBox = document.getElementById('resetModalAlert');
    ...
    if (result.success) {
        ...
        alertBox.textContent = result.message || 'Unable to reset password.';
