...
// Admins can edit any grade record (unlike the teacher endpoint, which is
// restricted to grades the teacher owns).
...
if (!$gradeId) {
    jsonResponse(false, 'Missing grade record.', [], 422);
}
...
try {
    $stmt = $pdo->prepare('SELECT grade_id FROM grades WHERE grade_id = :gid');
    $stmt->execute(['gid' => $gradeId]);
    if (!$stmt->fetch()) {
        jsonResponse(false, 'Grade record not found.', [], 404);
    }

    $stmt = $pdo->prepare('
        UPDATE grades SET grade = :grade, remarks = :remarks, updated_at = NOW() WHERE grade_id = :gid
    ');
    $stmt->execute([
        'grade'   => $gradeValue,
    ...
    error_log('Admin grade save failed: ' . $e->getMessage());
    jsonResponse(false, 'Unable to save grade. Please try again.', [], 500);
}

jsonResponse(true, 'Grade updated successfully.');
