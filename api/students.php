...
if ($firstName === '' || $lastName === '' || $email === '' || $course === '' || $loginId === '') {
...
if ($yearLevel < 1 || $yearLevel > 6) {
    jsonResponse(false, 'Year level must be between 1 and 6.', [], 422);
}
...
try {
    // Uniqueness checks (login_id and email must not belong to a different record).
    ...
    $stmt = $pdo->prepare('SELECT student_id FROM students WHERE email = :email AND student_id != :sid');
    $stmt->execute(['email' => $email, 'sid' => $studentId ?? 0]);
    if ($stmt->fetch()) {
    ...
    if ($isNew) {
        ...
        $stmt = $pdo->prepare('
            ...
            VALUES (:login_id, :password_hash, \'student\', TRUE)
            RETURNING user_id
        ');
        $stmt->execute([
            'login_id'      => $loginId,
        ...
        $stmt = $pdo->prepare('
            INSERT INTO students (user_id, first_name, last_name, email, course, year_level)
            VALUES (:user_id, :first_name, :last_name, :email, :course, :year_level)
        ');
        $stmt->execute([
            'user_id'    => $userId,
        ...
        // Update the login_id (and password if provided).
        if ($password !== '') {
        ...
        $stmt = $pdo->prepare('
            ...
            WHERE student_id = :sid AND user_id = :uid
        ');
        $stmt->execute([
            'first_name' => $firstName,
            'last_name'  => $lastName,
    ...
    jsonResponse(false, 'Unable to save student. Please try again.', [], 500);
}
